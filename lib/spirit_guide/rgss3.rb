  ...
  def initialize(data)
    ...
    *@elements = *data
    if @num_of_dimensions > 1
      @elements = if @xsize > 1
                    @elements.each_slice(@xsize).to_a
                  else
                    @elements.map { |element| [element] }
                  end
    end
    return unless @num_of_dimensions > 2

    @elements = if @ysize > 1
                  @elements.each_slice(@ysize).to_a
                else
                  @elements.map { |element| [element] }
                end
  end
...
class RPG::Map
  ...
  attr_accessor :display_name, :tileset_id, :width, :height, :scroll_type, :specify_battleback, :battleback1_name,
                :battleback2_name, :autoplay_bgm, :bgm, :autoplay_bgs, :bgs, :disable_dashing, :encounter_list, :encounter_step, :parallax_name, :parallax_loop_x, :parallax_loop_y, :parallax_sx, :parallax_sy, :parallax_show, :note, :data, :events
end
...
class RPG::EventCommand
  ...
  def unpack_names
    ...
    @parameters = []
    tmp.each do |i|
      @parameters << if i.is_a?(String)
                       RPG.unpack_str(i).dup
                     else
                       i
                     end
    end
...
class RPG::UsableItem < RPG::BaseItem
  ...
  attr_accessor :scope, :occasion, :speed, :animation_id, :success_rate, :repeats, :tp_gain, :hit_type, :damage,
                :effects
end
...
class RPG::State < RPG::BaseItem
  ...
  attr_accessor :restriction, :priority, :remove_at_battle_end, :remove_by_restriction, :auto_removal_timing,
                :min_turns, :max_turns, :remove_by_damage, :chance_by_damage, :remove_by_walking, :steps_to_remove, :message1, :message2, :message3, :message4
end
...
class RPG::Troop::Page::Condition
  ...
  attr_accessor :turn_ending, :turn_valid, :enemy_valid, :actor_valid, :switch_valid, :turn_a, :turn_b, :enemy_index,
...
class RPG::Animation
  ...
  attr_accessor :id, :name, :animation1_name, :animation1_hue, :animation2_name, :animation2_hue, :position,
...
class RPG::System
  ...
  def unpack_names
    ...
    @elements = []
    tmp.each do |i|
      @elements << RPG.unpack_str(i).dup unless i.nil?
    end
    tmp = []
    tmp = @skill_types.dup
    @skill_types = []
    tmp.each do |i|
      @skill_types << RPG.unpack_str(i).dup unless i.nil?
    end
    tmp = []
    tmp = @weapon_types.dup
    @weapon_types = []
    tmp.each do |i|
      @weapon_types << RPG.unpack_str(i).dup unless i.nil?
    end
    tmp = []
    tmp = @armor_types.dup
    @armor_types = []
    tmp.each do |i|
      @armor_types << RPG.unpack_str(i).dup unless i.nil?
    end
    tmp = []
    tmp = @switches.dup
    @switches = []
    tmp.each do |i|
      @switches << RPG.unpack_str(i).dup unless i.nil?
    end
    tmp = []
    tmp = @variables.dup
    @variables = []
    tmp.each do |i|
      @variables << RPG.unpack_str(i).dup unless i.nil?
    end
  end
...
module SpiritGuide
  # Functions for import and export of RGSS3 `.rvdata2` files.
  module RGSS3
    ...
    def rvdata2_to_json(rvdata2)
      if rvdata2.is_a?(Array)
        ...
        rvdata2.each_value(&:unpack_names) unless rvdata2.empty?
      else
    ...
    def restore_rvdata2(list)
      return list if [Integer, TrueClass, FalseClass].include?(list.class)
      return unless list.key?("json_class")
    ...
    module_function :rvdata2_to_json
    module_function :json_to_rvdata2
    module_function :restore_rvdata2
    module_function :iterate_setting_value
  end
end

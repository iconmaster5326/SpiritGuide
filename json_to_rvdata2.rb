...
def json_to_rvdata2(json)
  ...
  #あまり賢くない方法で対処（後で考える）
    ...
    if File.basename(json) == 'MapInfos.json'
      data_trans = {}
      data.each{|k, v|
  ...
  #p data_trans
  File.open(File.dirname(json)+"/"+File.basename(json,'.json')+'.rvdata2', 'wb') do |file|
...
files.each do |file| json_to_rvdata2(file) end
